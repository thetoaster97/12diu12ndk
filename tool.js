(function() {
  ...
  function getOneIdOs() {
    return navigator.userAgent.indexOf('Android') !== -1 ? 'AND' : 'IOS';
  }

  function getClientId(resortId) {
  ...
  function clearOneIdGuestData() {
    try {
      if (_oneIdClientId) {
        localStorage.removeItem(_oneIdClientId + '-PROD.guest');
      }
    } catch (e) {}
  }
  ...
  async function loadAttractionNames() {
    ...
    try {
      ...
      (d.results || []).forEach(function(item) {
        ...
        var numId = parseInt(String(raw).split(';')[0], 10);
